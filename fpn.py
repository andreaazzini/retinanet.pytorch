...
class PretrainedRetinaFPN(nn.Module):
    def __init__(self):
        super(PretrainedRetinaFPN, self).__init__()
        
        self.backbone = models.resnet50(pretrained=True).cuda()
    ...
    def forward(self, x):
        # Bottom-up
        conv1 = self.backbone[0](x)
        bn1 = self.backbone[1](conv1)
        relu = self.backbone[2](bn1)
        c1 = self.backbone[3](relu)
        c2 = self.backbone[4](c1)
